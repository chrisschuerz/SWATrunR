...
```{r}
obs_q <- obs_data[c("q_chol", "q_trais")] %>% 
...
```{r}
eval_q <- q_calibr$simulation %>% 
  map(., ~ select(.x, -date)) %>% 
  map2(., obs_q, ~ map_dbl(.x, function(sim,obs = .y$q){nse(sim, obs)})) %>% 
  bind_cols(.) %>% 
...
#### Dotty plots
```{r}
dotty_q <- plot_dotty(q_calibr$parameter, eval_q)
```
...
plot_timeseries(q_calibr$simulation$flow_2, obs_q$q_chol, eval_q$nse_2, ">", 0.4)
```

```{r, fig.height= 4, fig.width=7.5}
plot_timeseries(q_calibr$simulation$flow_3, obs_q$q_trais, eval_q$nse_3, ">", 0.4)
```
