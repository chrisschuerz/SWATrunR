...
The most recent develeopments of `SWATplusR` are available from the *development* branch of the package's GitHub repository. See the package's [Changelog](https://chrisschuerz.github.io/SWATplusR/news/index.html) for updates available in the development version. You can install the development version of `SWATplusR` available from the *dev* branch as follows:
