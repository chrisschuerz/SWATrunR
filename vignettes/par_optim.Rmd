...
We omitted any feedback of the optimization in that case. If you want to get som feedback of the optimization routine you can set the control parameter `trace = 6` in the list of control parameters above. The result of the optimization is saved in a list object. Relevant information is given by the list entries `opt_bfgs$par` that provides the final optimum parameter set and `opt_bfgs$value` that shows the optimized onbjective criterion, in our case the NSE:
...
To visualize the discharge time series with the found parameter set we have to rerun the `swat_model()` we defined above with the optimimum parameter set:
...
sce_plot <- q_sce$simulation$q_sim %>%
  mutate_if(is.numeric, funs(./8.64)) %>% 
  gather(., key = "variable", value = "discharge", -date)
...
ggplot() +
  ...
  geom_line(data = sce_plot, 
            aes(x = date, y = discharge, col = variable), lty = 2) +
