...
```{r, eval=FALSE}
q_out_plus <- run_swatplus(project_path = plus_path,
...
```{r, eval=FALSE}
q_out_2012 <- run_swat2012(project_path = path_2012,
...
```{r}
q_out_2012 <- readRDS(here::here("vignettes/datasets/q_out.rds"))
```
...
```{r, echo=FALSE}
wb_out <- readRDS(here::here("vignettes/datasets/wb_out.rds"))
```
...
```{r, eval=F}
q_out <- run_swatplus(project_path = path_plus,
                      output = define_output(file = "channel",
                                             variable = "flo_out",
                                             unit = 1),
...
### Simulations with many parameter setups
...
#>  Thread 4 of 4   Time elapsed: 2S   Time remaining: 0S    
#>  Completed 4 thread in 2S
#> Performing 8 simulations on 4 cores:
#>  Simulation 8 of 8   Time elapsed: 5S   Time remaining: 0S
#>  Completed 8 simulations in 5S
```

```{r, echo = F}
q_out <- readRDS(here::here("vignettes/datasets/q_out_8.rds"))
```
...
In many cases it is advantageous to save the simulations incrementally after each SWAT model evaluation, for instance when long simulation runs would be lost if the computer or the R instance crashes, or when the simulation results cannot be stored in the RAM storage at all.
...
In the example above these arguments are simply added to the `run_swatplus()` function call, to save the simulations in a data base instead of returning them back to R:
...
#>  Completed 4 thread in 2S
#> Performing 8 simulations on 4 cores:
#>  Simulation 8 of 8   Time elapsed: 6S   Time remaining: 0S
#>  Completed 8 simulations in 6S
```
